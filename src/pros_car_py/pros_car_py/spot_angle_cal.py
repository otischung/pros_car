...
class kinematics(Node):
    ...
    def timer_callback(self):
        if(not self.LB_target or not self.RB_target or not self.LF_target or not self.RF_target):
            return
    ...
    # --- for LB leg: ---
        ...
        print(f"p3: {p3}")
        print(f"p4: {p4}")
        angle_LB_shoulder = self.angle_between_vectors(vec1, vec2)
        ...
        vec1 = p1 - p2
        vec2 = p4 - p3
        angle_LB_thigh = self.angle_between_vectors(vec1, vec2)
        ...
        vec1 = p1 - p2
        vec2 = p3 - p2
        angle_LB_calf = self.angle_between_vectors(vec1, vec2)
    # --- for RB leg: ---
    ...
    # --- send to unity ---
        ...
        self.LB_target = [] # to ensure there is another data in next time.

        print(f"return_angle: {self.return_angles}")
        pub = JointTrajectoryPoint()
        return_angle_rad = [angle * math.pi / 180 for angle in self.return_angles]
