...
class SpotKeyboardController(Node):
    def __init__(self, stdscr, vel: float = 10):
        ...
        self.move_publisher_ = self.create_publisher(
            Float32MultiArray,
            'LF_move',
            10
        )
    ...
    def handle_key_s(self):
        return_dims = self.generate_foot_trajectory()
        for i in range(len(return_dims)):
            msg = Float32MultiArray()
            msg.data = return_dims[i]
